...
// 충전소 정보를 가져오는 함수
// Django 백엔드에 KEPCO용으로 "번역된" metroCd와 cityCd를 전달합니다. 
// 구함수, 작동 자체는 되나 아예 새로 짰음.
/*
function fetchStations(currentLat, currentLon, metroCd, cityCd) {
  const listContainer = document.getElementById('list-container');
  listContainer.innerHTML = '충전소 정보 로딩 중...';
  ...
  let fetchUrl = `/stations/?metroCd=${metroCd}`;
  // cityCd가 비어있지 않다면 URL에 추가 (views.py에서 cityCd가 없으면 무시하도록 만들어둠)
  if (cityCd && cityCd.trim()) { 
    fetchUrl += `&cityCd=${cityCd}`;
  }
      ...
      if (stations && stations.length > 0) {
        ...
        stations.forEach(station => {
          if (station.lat && station.lon) { 
            const distance = getDistance(currentLat, currentLon, parseFloat(station.lat), parseFloat(station.lon));
            placeDistances.push({ 
              name: station.stnPlace || station.name, 
              distance: distance 
            });

            if (distance < nearestDistance) {
              nearestDistance = distance;
              nearestPlaceData = station;
            }

            const marker = new kakao.maps.Marker({
              position: new kakao.maps.LatLng(parseFloat(station.lat), parseFloat(station.lon)),
              map: map,
              image: stationMarkerImage
            });

            const infoWindowContent = `<div style="padding:5px 10px; font-size:14px; white-space: nowrap;">${station.stnPlace || station.name} - ${distance.toFixed(2)} km</div>`;
            const infowindow = new kakao.maps.InfoWindow({ content: infoWindowContent });

            kakao.maps.event.addListener(marker, 'mouseover', () => infowindow.open(map, marker));
            kakao.maps.event.addListener(marker, 'mouseout', () => infowindow.close());
            kakao.maps.event.addListener(marker, 'click', () => {
              const link = `https://map.kakao.com/link/to/${station.stnPlace || station.name},${station.lat},${station.lon}`;
              window.open(link, '_blank');
            });
          } else {
            placeDistances.push({
                name: `${station.stnPlace || '이름없음'} (${station.stnAddr || '주소없음'})`,
                distance: null 
            });
            console.warn(`좌표 없는 충전소: ${station.stnPlace}, 주소: ${station.stnAddr}. 지도에 표시할 수 없습니다. 주소 지오코딩 필요.`);
          }
        });
        
        if (nearestPlaceData && nearestPlaceData.lat && nearestPlaceData.lon) {
            const nearestMarker = new kakao.maps.Marker({
                position: new kakao.maps.LatLng(parseFloat(nearestPlaceData.lat), parseFloat(nearestPlaceData.lon)),
                map: map,
                image: new kakao.maps.MarkerImage( 
                    'https://cdn-icons-png.flaticon.com/512/3103/3103446.png',
                    new kakao.maps.Size(40, 40) 
            ...
            const nearestInfoWindow = new kakao.maps.InfoWindow({
                content: `<div style="padding:5px 10px; font-size:14px; font-weight:bold; white-space: nowrap;">🔋 가장 가까운 충전소 - ${nearestPlaceData.stnPlace || nearestPlaceData.name} (${nearestDistance.toFixed(2)} km)</div>`
            });
            nearestInfoWindow.open(map, nearestMarker);
            kakao.maps.event.addListener(nearestMarker, 'click', () => {
                const link = `https://map.kakao.com/link/to/${nearestPlaceData.stnPlace || nearestPlaceData.name},${nearestPlaceData.lat},${nearestPlaceData.lon}`;
                window.open(link, '_blank');
            });
        }

        placeDistances.sort((a, b) => {
            if (a.distance === null) return 1; 
            if (b.distance === null) return -1;
            return a.distance - b.distance;
        });
        ...
        placeDistances.forEach((item, index) => {
          const div = document.createElement('div');
          div.className = 'station-item';
          div.textContent = `${index + 1}. ${item.name}` + (item.distance !== null ? ` - ${item.distance.toFixed(2)} km` : ' (거리 정보 없음)');
          div.addEventListener('click', () => {
            const queryName = item.name.split('(')[0].trim(); 
            const url = `https://search.naver.com/search.naver?query=${encodeURIComponent(queryName)}`;
            window.open(url, '_blank');
          });
          listContainer.appendChild(div);
